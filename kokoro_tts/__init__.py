...
# Local imports
from .cpu_affinity import set_e_core_affinity
...
def apply_ashley_voice(audio, sr):
    ...
    Returns:
        Pitch-shifted audio matching Azure Ashley voice
    """
    return pitch_shift_audio(audio, sr, n_steps=3)
...
# This function is used in the smoke test to validate voice selection and blending logic
# Change this blend to have different mix
def validate_voice(voice, kokoro):
    ...
    Built-in presets: "ashley_neuro" (af_sarah:70,af_bella:30 with +6 semitone pitch)
    """
    try:
...
def process_chunk_sequential(chunk: str, kokoro: Kokoro, voice, speed: float, lang: str,
                           retry_count=0, debug=False, apply_pitch_shift=False) -> tuple[list[float] | None, int | None]:
    ...
    try:
        ...
        # Apply pitch shift for ashley_neuro (+6 semitones ≈ 25% higher pitch)
        if apply_pitch_shift and samples is not None:
            samples = apply_ashley_voice(np.array(samples), sample_rate)
...
def convert_text_to_audio(input_file, output_file=None, voice=None, speed=1.0, lang="en-us", 
    ...
    try:
        ...
        if voice:
            original_voice_request = voice  # Track original request for ashley_neuro
            voice = validate_voice(voice, kokoro)
        else:
            original_voice_request = None
...
def main():
    ...
    # Lock to E-cores for efficiency on mixed-core CPUs (i9-13900KS, etc.)
    set_e_core_affinity()
